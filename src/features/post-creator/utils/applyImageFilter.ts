...
export const applyFilterToImage = async (photo: PhotoState): Promise<File> => {
   ...
   const blob = await new Promise<Blob>(resolve => canvas.toBlob(resolve as any, 'image/jpeg', 0.9)) //конвертируем в бинарные данные формата 'image/jpeg' и сжатием 90% (рекомендуют именно 90% мол качество не сильно страдает, но вес значительно уменьшится)
